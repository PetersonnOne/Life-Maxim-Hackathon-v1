...
function InteractiveSession({ id, profiles, onBack, onUserMode }: { id: Id<"interactiveSessions">; profiles: Doc<"profiles">[]; onBack: () => void; onUserMode: () => void }) {
  ...
  async function start() {
    ...
    try {
      if (!navigator.mediaDevices?.getUserMedia || !window.RTCPeerConnection) throw new Error("Use a browser supporting WebRTC on HTTPS or localhost.");
      ...
      dc.onmessage = event => {
        ...
        if (data.type === "session.delegation.created") { setDelegated(true); const delegationId = data.delegation?.id; if (typeof delegationId === "string" && !seen.current.has(delegationId)) { seen.current.add(delegationId); dc.send(JSON.stringify({ type: "session.commentary.append", event_id: crypto.randomUUID(), delegation_id: delegationId, content: "No backend work has started. Please use Prepare brief, review it, and confirm the handoff in the interface." })); } }
      ...
      timer.current = setTimeout(() => { setError("Voice did not become ready. No automatic retry was made."); void stopRef.current(); }, 15000);
  ...
  return <section className="interactive-mode"><div className="interactive-toolbar"><Button variant="ghost" onClick={async () => { await stop(); onBack(); }}><ArrowLeft size={16}/>Discussions</Button><Button variant="outline" onClick={async () => { await stop(); onUserMode(); }}>Back to User Mode</Button></div>
    <div className="page-heading"><div><span className="eyebrow">INTERACTIVE MODE · {profile?.name ?? "Loading profile"}</span><h1>{saved?.title || "Make room for a conversation."}</h1><p>GPT-Live-1 listens and clarifies. Terra and Luna handle the deeper work.</p></div></div>
